...
*********************************************************/
import  __GLUE_EXEC__ from '../combine/glue'
import __CODEC_EXEC__ from '../bin/prod'
...
if(!WORKER_ENABLED){
    ...
    H264Codec.prototype.skipBodyDecode = function(buffer){
        if(this.Module){
            this.Module.onmessage({
                type: 'skipBodyDecode',
                buffer: buffer,
    ...
    H264Codec.prototype.useOffscreen = function () {
        if (this.Module) {
            this.Module.onmessage({
                type: 'useOffscreen'
            });
