...
function __GLUE_EXEC__(Module){
  ...
  (isWorker ? self : window)[bridgeName] = {
    ...
    onVideoData: function(data) {
      ...
      if (useOffscreen) {
        drawer.drawNextOutputPicture(data.width, data.height, output);
        const bitmap = drawer.$canvas.transferToImageBitmap();
        Module.postMessage(
          {
            type: "video",
            data: {
  ...
  Module.onmessage = function(msg) {
    ...
    switch (data.type) {
      ...
      case "skipBodyDecode": {
        ...
        Module.audioTimestamps = [];
        Module.videoTimestamps = [];
        Module._codecSkipBodyDecode(data, buffer.length);
